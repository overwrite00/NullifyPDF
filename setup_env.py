...
def setup_environment():
    ...
    if os.path.exists(venv_dir):
        print("[*] Rimozione vecchio ambiente virtuale corrotto in corso...")
    ...
    # 4. Aggiorna pip e installa le dipendenze
    print("[*] Aggiornamento di pip...")
    subprocess.run([venv_python, "-m", "pip", "install", "--upgrade", "pip", "-q"])

    print("[*] Installazione delle librerie (customtkinter, PyMuPDF, pillow)...")
    subprocess.run(
        [venv_python, "-m", "pip", "install", "customtkinter", "PyMuPDF", "pillow"]
    )
